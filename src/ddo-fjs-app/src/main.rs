...
impl Problem for Fjs {
        ...
        var: Variable, state: &Self::State, f: &mut dyn DecisionCallback)
    {
        ...
        loop {
            ...
            loop {
                ...
                idx += 1;
                if idx == self.J || self.P[idx] != start {
                    break;
                }
            }
            // at this point, we have all APPs between 'os' and 'idx' exclusive

            for job in os..idx {
                if begun == true && state.u[job] == -1 {
                    continue;
                }
    ...
    fn is_impacted_by(&self, _var: Variable, state: &Self::State) -> bool {
        let mut idx = 0;
        for x in 0..=self.P.last().copied().expect("Oops") {
            let mut job_done = false;
            while idx < self.P.len() && self.P[idx] == x {
                if (state.u[idx] + 1) as usize == self.j[idx] {
                    job_done = true;
                }
                idx += 1;
            }
...
fn main() {
    ...
    let mut solver = ParNoCachingSolverPooled::custom(//SeqCachingSolverFc::new(
        &problem,
        &relaxation,
        &ranking,
        width.as_ref(),
...
> {
    ...
    let (J, M, j, O, P) = if toks.len() == 1 {
        ...
        (J, M, j, opts, Vec::from_iter(0..J))
    } else {
        // FLAT+APP format
        let header: Vec<usize> = toks.iter()
            .take(2)
            .map(|&s| s.parse())
            .collect::<Result<_,_>>()?;
        if header.len() < 2 {
            return Err("flat header: need J M".into());
        }
        let (J, M) = (header[0], header[1]);

        // FLAT+APP‐only parsing
        // ---------------------------------------

        // We'll collect:
        //   j_out:   #ops per APP-job
        //   O_out:   Vec of op-alternatives per APP-job
        //   P:       original-job ID per APP-job
        let mut j_out = Vec::new();
        let mut O_out = Vec::new();
        let mut P     = Vec::new();

        for job_id in 0..J {
            // === 1) read the augmented job‐header line ===
            //    format: A t [ a m p ... ] for each of t base ops
            let parts: Vec<usize> = get_line(&lines, &mut pos, "job+APP header")?
                .split_whitespace()
                .map(str::parse)
                .collect::<Result<_,_>>()?;
            let mut it = parts.into_iter();
            let A = it.next().unwrap();         // number of APPs
            let t = it.next().unwrap();         // total base‐operations

            // Parse all base alternatives for each op
            // Index 1..=t
            let mut base_alts: Vec<Vec<(usize,usize)>> = Vec::with_capacity(t+1);
            for _ in 0..t {
            ...
            // Read each APP and emit as a separate job
            for _ in 0..A {
                // APP line: l idx1 idx2 ... idxl
                let tokens: Vec<usize> = get_line(&lines, &mut pos, "APP line")?
                    .split_whitespace()
                    .map(str::parse)
                    .collect::<Result<_,_>>()?;
                let mut pit = tokens.into_iter();
                let l = pit.next().unwrap();
                let indices: Vec<usize> = pit.take(l).collect();

                // Record mapping and job size
                P.push(job_id);
                j_out.push(l);

                // Build ops for this APP-job using full alternatives
                let mut ops: Vec<Vec<(usize,usize)>> = Vec::with_capacity(l);
                for op_idx in indices {
                    // clone the original alternatives list
                    ops.push(base_alts[op_idx].clone());
                }
                O_out.push(ops);
            }
        }

        // Now replace your outputs:
        let J_prime = P.len();      // total alternative‐jobs
        let j = j_out;
        let O = O_out;
        // you can return P alongside (J_prime, M, j, O)
        (J_prime, M, j, O, P)
    };
