...
impl Problem for Fjs {
        ...
        state: &Self::State, dec: Decision) -> Self::State
    {
        ...
        let mut rng = rng();
        for job in 0..self.J {
            for raw in clone.v[job]+1..=clone.j[job].try_into().unwrap() {
                let task = raw as usize;
                // only if this job still has a task task waiting
                if task < self.j[job] {
                    let options = &self.O[job][task];
                    // now `choose` is in scope
                    if let Some(&(m, p)) = options.choose(&mut rng) {
                        clone.f[m] = clone.f[m].max(clone.t[job]) + p;
                        clone.t[job] = clone.f[m];
                    }
                }
            }
        }
...
impl Relaxation for FjsRelax {
    ...
    fn merge(&self, states: &mut dyn Iterator<Item=&Self::State>) -> Self::State {
        ...
        for job in 0..merged.J {
            for raw in merged.v[job]+1..=merged.j[job].try_into().unwrap() {
                let task = raw as usize;
                // only if this job still has a task task waiting
                if task < merged.j[job] {
                    let options = &merged.O[job][task];
                    // now `choose` is in scope
                    if let Some(&(m, p)) = options.choose(&mut rng) {
                        clone.f[m] = clone.f[m].max(clone.t[job]) + p;
                        clone.t[job] = clone.f[m];
