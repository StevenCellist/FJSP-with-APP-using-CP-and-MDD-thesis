...
def plot_machine_gantt(
    ...
    for idx, task_data in enumerate(solution.tasks):
        a = 0.25 if data.tasks[idx].name == "(1, 2)" else 0.75
        a = 0.75
        kwargs = {
    ...
    ax.set_xlabel(x_label) if x_label else ax.set_xlabel("Time")
    ax.set_title(title) if title else ax.set_title("Solution")
